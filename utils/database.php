...
class Database
{
     ...
     * @param string $password (Mot de passe, par défaut "Alt6WH9t%W")
     */
    public function __construct($db_name, $db_host = "localhost", $user_name = "mdaszczynski", $password = "Alt6WH9t%W")
    {
    ...
    private function getPDO()
    {
        if ($this->bdd === null) {
            try {
                ...
                die("Erreur de connexion à la base de données : $exeption");
            }
        }
        return $this->bdd;
    }
     ...
     * @return string ($req => la requete preparer et executer ) 
     */
    public function updateBDD($table, $data = [], $id)
    {
        $model = [];
        // on creer le tableau de parametre
        $param = [":id" => $id];
